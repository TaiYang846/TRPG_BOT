...
功能：
1. /劇本 [名稱]     -> 查詢指定劇本的詳細資訊
2. /劇本列表         -> 用下拉選單列出所有目前收錄的劇本名稱（可翻頁），選了就直接顯示資訊
...
8. /人數 [人數]      -> 依照遊玩人數，用下拉選單列出支援該人數遊玩的劇本（可翻頁）
...
另外還有一個獨立的功能模組：keyword_replies.py（關鍵字自動回應），
跟劇本查詢功能完全分開寫、互不影響，詳見該檔案開頭的說明。
"""
...
intents.message_content = True  # 關鍵字自動回應需要讀取訊息內容，才需要這個權限
client = discord.Client(intents=intents)
tree = app_commands.CommandTree(client)

# 掛載「關鍵字自動回應」功能，程式碼完全獨立在 keyword_replies.py 裡
...
def build_scenario_options(matches: list) -> list:
    """把一批 (名稱, 資料) 轉成下拉選單的選項列表"""
    options = []
    for name, data in matches:
        desc_parts = []
        if data.get("author"):
            desc_parts.append(f"作者：{data['author']}")
        player_text = format_player_count(data)
        if player_text:
            desc_parts.append(player_text)
        description = "，".join(desc_parts)[:100] or None
        options.append(discord.SelectOption(label=name[:100], description=description))
    return options


class ScenarioSelect(discord.ui.Select):
    """劇本下拉選單，選了哪個劇本就直接顯示那個劇本的完整資訊卡片"""

    def __init__(self, options: list):
        super().__init__(
            placeholder="請選擇要查看的劇本…",
            min_values=1,
            max_values=1,
            options=options,
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        data = SCENARIOS.get(chosen_name)

        if not data:
            await interaction.response.send_message(
                f"「{chosen_name}」這個劇本的資料好像被移除或改名了😵，請重新查詢一次。",
                ephemeral=True,
            )
            return

        await interaction.response.send_message(embed=build_scenario_embed(chosen_name, data))


class ScenarioSelectView(discord.ui.View):
    """
    包住 ScenarioSelect 的 View，支援分頁：
    - 一頁最多顯示 25 個劇本（Discord 選單本身的上限）
    - 超過 25 個時，會自動出現「◀ 上一頁」「下一頁 ▶」按鈕可以翻頁
    - 3 分鐘沒人操作就會自動失效
    """

    PAGE_SIZE = 25

    def __init__(self, matches: list, header: str):
        super().__init__(timeout=180)
        self.matches = matches
        self.header = header
        self.page = 0
        self.total_pages = max(1, (len(matches) + self.PAGE_SIZE - 1) // self.PAGE_SIZE)

        self.select = ScenarioSelect(build_scenario_options(self._current_page_matches()))
        self.add_item(self.select)

        if self.total_pages > 1:
            self.prev_button = discord.ui.Button(
                label="◀ 上一頁", style=discord.ButtonStyle.secondary, disabled=True
            )
    ...
    def _current_page_matches(self) -> list:
    ...
    def content(self) -> str:
        text = self.header
        if self.total_pages > 1:
            text += f"\n（第 {self.page + 1} / {self.total_pages} 頁，共 {len(self.matches)} 個劇本）"
        return text

    async def _refresh(self, interaction: discord.Interaction):
        self.select.options = build_scenario_options(self._current_page_matches())
...
@tree.command(name="劇本列表", description="用下拉選單列出所有目前收錄的劇本")
async def scenario_list(interaction: discord.Interaction):
    if not SCENARIOS:
        await interaction.response.send_message("目前還沒有收錄任何劇本喔！")
        return

    matches = list(SCENARIOS.items())
    view = ScenarioSelectView(matches, "📚 目前收錄的劇本，請從下面選單選一個看看～")
    await interaction.response.send_message(view.content, view=view)


@tree.command(name="人數", description="依照遊玩人數，用下拉選單列出符合的劇本")
